...
class TermLifeInsurance(object):
    ...
    def expected_expense_df(self, t_end):
        ...
        expense_df = pandas.DataFrame([(i,j) for i,j in zip(expense['t'], expense['expected'])], columns = ["t", "expense"])
        return expense_df
    ...
    def minimum_level_premium(self, annuity = 'immediate', point = "issue"):
        if point == "issue":
            point = self.x
        exp = []
        if annuity == 'immediate':
            for t in range(point+1, self.t_end+1):
                payments =  [Premium(i, 1) for i in range(point+1, t+1)]
                ...
                if t==self.t_end:
                    ...
                    exp.append(payments_pv*payments[-1].payment_prob(point, self.table, next_payment_time = t+1))
        elif annuity == 'due':
            for t in range(point, self.t_end):
                payments =  [Premium(i, 1) for i in range(point, t+1)]
                payments_pv = sum([p.value_at(point, self.interest_rates) for p in payments])
                if t==(self.t_end-1):
                    exp.append(payments_pv*payments[-1].payment_prob(point, self.table))
                else:
...
class TermLifeAnnuity(object):
    ...
    def expected_payments_df(self, t_end):
        income = self.expected_payments(t_end)        
        income_df = pandas.DataFrame([(i,j) for i,j in zip(income['t'], income['expected'])], columns = ["t", "income"])
        return income_df
...
class ActuarialModel(object):
    ...
    def __init__(self, insurance, annuity, policy_year, policy_month=1):
        ...
        if annuity == "level-immediate":
            premium = self.insurance.minimum_level_premium(annuity = "immediate")
            self.annuity = TermLifeAnnuity(self.insurance.x, self.insurance.term, \
                                           [Premium(i, premium) for i in range(self.insurance.x + 1, self.insurance.x + self.insurance.term + 1)], \
                                           self.insurance.interest_rates, \
                                           self.insurance.table)
        elif annuity == "level-due":
            premium = self.insurance.minimum_level_premium(annuity = "due")
            self.annuity = TermLifeAnnuity(self.insurance.x, self.insurance.term, \
                                           [Premium(i, premium) for i in range(self.insurance.x, self.insurance.x + self.insurance.term)], \
                                           self.insurance.interest_rates, \
                                           self.insurance.table)
        else:
    ...
    def income_expense_df(self, t_end):
        income, expense = self.income_expense(t_end)
        income_df = pandas.DataFrame([(i,j) for i,j in zip(income['t'], income['expected'])], columns = ["t", "income"])
        expense_df = pandas.DataFrame([(i,j) for i,j in zip(expense['t'], expense['expected'])], columns = ["t", "expense"])
    ...
    def cashflow_df(self, t_end, frequency_weight = 1, collective_interest_rates = None):
        ...
        if (collective_interest_rates == None):
            collective_interest_rates = [fm.InterestRate(0, 0, "annual")]
        collective_premiums = [p.collective_fund_premium(False) for p in self.annuity.payments]
        collective_fund_annuity = TermLifeAnnuity(self.insurance.x, self.insurance.term, collective_premiums, \
                                                  collective_interest_rates, self.insurance.table)        
